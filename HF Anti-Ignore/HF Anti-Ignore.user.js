...
// ==/UserScript==
// ------------------------------ Change Log ----------------------------
// version 1.2.0: Updated to work on HF post MyBB 1.8
...
// ------------------------------ SETTINGS ------------------------------
var debug = false;
var totalPosts = 36613335;
var showBlockedPosts = false;
var showBlockedProfiles = true;
// ------------------------------ Page Load -----------------------------
if (window.location.href.includes("hackforums.net/member.php?action=profile&uid=") && showBlockedProfiles) {
    runonProfile();
}
else if (window.location.href.includes("hackforums.net/showthread.php?tid=") && showBlockedPosts) {
    runonShowThread();
}

function runonShowThread() {
...
function runonProfile() {
    $("table").each(function (index) {
        if ($(this).find(".smalltext strong").text() == "Ignore Error") {
            ...
            // Append table
            $("#content").append("<br>").append($("<table>").css("width", "50%").css("margin-right", "auto").css("margin-left", "0px").attr('id', 'antiIgnore').attr('border', 0).attr('cellspacing', 1)
                ...
                .done(function (msg) {
                    ...
                    $(resultTables).each(function (resultIndex) {
                        ...
                        if (($(this).find(".thead div:eq(1) strong").text()) === "Member List") {
                            ...
                            userPostCount = $(desiredRow).find("td:eq(4)").text();
                            userThreadCount = $(desiredRow).find("td:eq(5)").text();
                            userReputation = $(desiredRow).find("td:eq(6)").text();
                            if (userReputation > 0)
                            ...
                            if (debug) {
                                ...
                                console.log("Thread Count: " + userThreadCount);
